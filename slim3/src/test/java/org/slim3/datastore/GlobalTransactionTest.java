...
public class GlobalTransactionTest extends AppEngineTestCase {
     ...
     */
    @Test
    public void isLockedOver30secondsWhenStatusIsRolledBack() throws Exception {
        Key putKey = Datastore.createKey("Hoge", 1);
        Entity entity = new Entity(putKey);
        gtx.put(entity);
        gtx.startTransactionInternally();
        ...
        gtxEntity.setProperty(
            GlobalTransaction.STATUS_PROPERTY,
            GlobalTransaction.ROLLED_BACK_STATUS);
        Datastore.put(gtxEntity);
        lockEntity.setProperty(
            GlobalTransaction.GLOBAL_TRANSACTION_KEY_PROPERTY,
            gtxEntity.getKey());
     ...
     */
    @Test
    public void isLockedOver30secondsWhenStatusIsApplied() throws Exception {
        Key putKey = Datastore.createKey("Hoge", 1);
        Entity entity = new Entity(putKey);
        gtx.put(entity);
        gtx.startTransactionInternally();
        ...
        gtxEntity.setProperty(
            GlobalTransaction.STATUS_PROPERTY,
            GlobalTransaction.APPLIED_STATUS);
        Datastore.put(gtxEntity);
        lockEntity.setProperty(
            GlobalTransaction.GLOBAL_TRANSACTION_KEY_PROPERTY,
            gtxEntity.getKey());
    ...
    public void deleteInTx() throws Exception {
        ...
        TxEntry entry = gtx.entrySet.iterator().next();
        assertThat(entry.writeJournal(), is(true));
        assertThat(GlobalTransaction
            .deleteInTx(entry.lockKey, entry.journalKey), is(true));
