...
public class TokenUtilTest extends TestCase {
    ...
    public void testIsTokenValid() throws Exception {
        TokenUtil.saveToken();
        request.addParameter(Constants.TOKEN_KEY, request.getSession()
                .getAttribute(Globals.TRANSACTION_TOKEN_KEY).toString());
        assertTrue(TokenUtil.isTokenValid(true));
        assertFalse(TokenUtil.isTokenValid(true));
    }
}