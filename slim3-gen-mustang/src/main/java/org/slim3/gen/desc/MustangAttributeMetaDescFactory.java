...
import org.slim3.gen.processor.MustangLogger;
import org.slim3.gen.processor.MustangOptions;
import org.slim3.gen.util.MustangElementUtil;
 ...
 * 
 */
public class MustangAttributeMetaDescFactory {

    /** the processing environment */
    protected final ProcessingEnvironment processingEnv;

    /**
     * Creates a new {@link MustangAttributeMetaDescFactory}.
     * 
     * @param processingEnv
     *            the processing environment
     */
    public MustangAttributeMetaDescFactory(ProcessingEnvironment processingEnv) {
    ...
    public AttributeMetaDesc createAttributeMetaDesc(
            VariableElement attributeElement) {
        if (attributeElement == null) {
            throw new NullPointerException(
                "The fieldElement parameter is null.");
        }
        if (!isPersistent(attributeElement)) {
            if (isInstanceVariable(attributeElement)
                && !isNotPersistent(attributeElement)) {
                MustangLogger
                    .warning(processingEnv, attributeElement, MessageFormatter
                        .getSimpleMessage(MessageCode.SILM3GEN0010));
            }
            return null;
        }
        if (isNestedType(attributeElement)) {
            return null;
        }
        Iterator<String> classNames =
            new ClassNameCollector(attributeElement.asType())
                .collect()
                .iterator();
        AttributeMetaDesc attributeMetaDesc = new AttributeMetaDesc();
        attributeMetaDesc.setName(attributeElement.getSimpleName().toString());
        if (isEmbedded(attributeElement)) {
    ...
    protected boolean isPersistent(Element attributeElement) {
        return MustangElementUtil.getAnnotationMirror(
            attributeElement,
