...
public class DaoTestCaseGenerator implements Generator {
	...
	public void generate(Printer p) {
		...
		p.println();
		p.println("public class %s%s extends %s {", daoDesc.getSimpleName(),
				Constants.TEST_SUFFIX, ClassUtil.getSimpleName(daoDesc
						.getTestCaseSuperclassName()));
		p.println();
		p.println("    public void test() throws Exception {");
		p.println("        %1$s %2$s = new %1$s();", daoDesc.getSimpleName(),
