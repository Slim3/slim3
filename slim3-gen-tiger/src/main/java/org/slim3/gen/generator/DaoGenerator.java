...
public class DaoGenerator implements Generator {
	...
	public void generate(Printer p) {
		...
		p.println();
		p.println("public class %s extends %s<%s> {", daoDesc.getSimpleName(),
				ClassUtil.getSimpleName(daoDesc.getSuperclassName()), ClassUtil
		...
		p.println("    protected SelectQuery<%s> from() {", ClassUtil
				.getSimpleName(daoDesc.getModelClassName()));
		p.println("        return new SelectQuery<%s>(pm, m.getModelClass());",
