...
public class PrimitiveArrayAttrsModelTest {
	...
	public void modelToJson_null() throws Exception{
		Assert.assertEquals("{}", meta.modelToJson(new PrimitiveArrayAttrsModel()));
	}

	@Test
	public void jsonToModel() throws Exception{
		PrimitiveArrayAttrsModel m = meta.jsonToModel(
				"{\"bytes\":\"aGVsbG8=\"}");
		Assert.assertArrayEquals("hello".getBytes(), m.getBytes());
	}

	private PrimitiveArrayAttrsModelMeta meta = PrimitiveArrayAttrsModelMeta.get();
}
